...
{{
  ...
  solve = dd
  variable = $Vb
  start =  1.5  #Это старт и начало отрисовки ВАХ по ОХ
  stop = 0 
  steps = 15 # количество точек. в эксперименте чаще всего идёт шаг 0,1V немного влияет на скорость счета
  plot_data = true
}}
...
'''
    with open(f"{out_dir}/pero_solarcell.tib", 'w') as f:
...
'''
    with open(f"{out_dir}/C60_.dat", 'w') as f:
        f.write(file_contents)
...
T_4_1_c = 0.5
'''
    with open(f"{out_dir}/FAPbBr2I.dat", 'w') as f:
        f.write(file_contents)
...
'''
    with open(f"{out_dir}/pedot_.dat", 'w') as f:
        f.write(file_contents)
